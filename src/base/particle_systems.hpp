...
public:
	ClothSystem(unsigned x, unsigned y, bool t) : x_(x), y_(y), tear(t) { reset(); }
	State					evalF(const State&) const override;
#ifdef EIGEN_SPARSECORE_MODULE_H
	void					evalJ(const State&, SparseMatrix& result, bool initial) const override;
#endif
...
class GeneratorSystem : public ParticleSystem {
public:
	GeneratorSystem(unsigned n, unsigned t) : n_(n), t_(t) { /*reset();*/ }
	State					evalF(const State&) const override;
#ifdef EIGEN_SPARSECORE_MODULE_H
	void					evalJ(const State&, SparseMatrix& result, bool initial) const override;
#endif
