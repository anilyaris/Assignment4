...
App::App(void)
	...
	spring_system_(),
	pendulum_system_(10),
	cloth_system_(20, 20, tearing_),
	generator_system_(5, 500),
	initial_implicit_(false)
{
	...
	common_ctrl_.addToggle((S32*)&ps_type_, GENERATOR_SYSTEM, FW_KEY_5, "Particle generator (5)", &system_changed_);
	...
	common_ctrl_.addToggle(&tearing_, FW_KEY_R, "Toggle cloth tearing (R)", &tearing_changed_);
#ifdef COMPUTE_CLOTH_MODULE
...
bool App::handleEvent(const Window::Event& ev) {
	if (system_changed_) {
		...
		case GENERATOR_SYSTEM:
			ps_ = &generator_system_; break;
#ifdef COMPUTE_CLOTH_MODULE
		case COMPUTE_CLOTH:
	...
	if (tearing_changed_) {
		cloth_system_.setTear(tearing_);
		tearing_changed_ = false;
	}
